## Resumen
* [] Quitar negrita
* [x] Abstract igual en ambos idiomas

## Glosario
* Pag 11. [x] Corregir glosario (Estrato)

## Introducción
* Pag 15. [] Agregar fuentes en algunas afirmaciones
* Pag 15. [~] Revisar las normas para las citaciones
* Pag 15. [x] Arreglar "Contribuír a reducir esta brecha"
* Pag 15. [x] Definir que es un punto de interés.


## Problema de investigación
* Pag 17. [x] Definir que es una "implementación irregular"

## Objetivos
* Pag 19. [x] Revisar los objetivos

## Estado del arte
* Pag 21. [x] Eliminar resaltos con negrilla
* Pag 21. [x] Unir redacción en un mismo párrafo
* Pag 22. [x] Unir parrafos
* Pag 22. [x] Corregir referencia a tabla
* Pag 22. [] Incluír información de modelos aplicados en latam

## Métodología

* Pag 25. [] Definir coherencia estructural
* Pag 25. [x] Eliminar el archivo pkl del diagrama
* Pag 26. [x] Corregir la afirmación de la asímetria positiva
* Pag 27. [x] Indicar que percentil se consideran los datos eliminados
* Pag 27. [x] Definir agrupación espacial consistente
* Pag 27. [x] Aclaración de limites administrativos
* Pag 27. [x] Por que se seleccionaron los radios de 100, 200, 500 y 1000 metros
* Pag 28. [x] Cuales son las variables derivadas
* Pag 28. [] Como evaluar redundancias espaciales?
* Pag 28. [x] Unir parrafos
* Pag 28. [x] Revisar referencia a tabla
* Pag 29. [x] Mejorar tabla de POIs
* Pag 29. [x] Ya revisado, definición de POIs.
* Pag 29. [x] No definir el acrónimo multiples veces.
* Pag 30. [x] Agregar una descripción de los datos que se agregaron.
* Pag 30. [x] Incluír soporte para la afirmación que se hizo (eliminada)
* Pag 31. [x] Definir variables del entorno urbano
* Pag 31. [x] Indicar que se hizo otra busqueda de hiperparámetros
* Pag 31. [] Revisar si hay otros aportes en otros lugares del mundo...
* Pag 32. [x] Aclarar que se hizo el mismo fold para probar los modelos.
* Pag 32. [x] Revisar lo de balance adecuado.
* Pag 34. [x] Revisar el diagrama de arquitectura.






