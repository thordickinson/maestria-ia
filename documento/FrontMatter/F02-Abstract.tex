...
Multiple predictive models (Linear Regression, Ridge, Lasso, SVR, Random Forest, LightGBM, and XGBoost) were trained and compared, evaluated through ten-fold cross-validation. The baseline model, built using only structural variables, achieved a mean RMSE of 0.1514 and $R^{2} = 0.965$, statistically outperforming the enriched models. Location and floor area emerged as the main determinants of price, with latitude, longitude, and UPZ implicitly capturing socioeconomic and territorial patterns of the city.

The model enriched with geospatial variables obtained an RMSE of 0.1705 and $R^{2} = 0.956$, while the optimized enriched model—using a reduced subset of variables selected through SHAP—achieved an RMSE of 0.1584. Although these variables did not improve predictive accuracy, they did enhance interpretability of the urban context and confirmed the structural relevance of location in price formation.
