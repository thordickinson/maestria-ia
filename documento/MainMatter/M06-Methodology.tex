...
\begin{figure}[ht]
    ...
    \caption{Conteo de POIs en radios de proximidad alrededor de un inmueble ejemplo.}
...
\hline
LightGBM          & 0.170 & 0.005 & 0.126 & 0.956 \\
XGBoost           & 0.170 & 0.005 & 0.126 & 0.956 \\
...
El modelo se entrenó utilizando el mismo pipeline y los mismos hiperparámetros del modelo enriquecido, de modo que cualquier cambio en el desempeño se debiera exclusivamente a la selección de variables. La validación cruzada de diez pliegues produjo un RMSE promedio de 0.158 (std 0.008), un MAE de 0.114 y un $R^2$ de 0.960, lo que ubica al modelo optimizado entre el modelo base y el modelo enriquecido en términos de precisión.

Para evaluar si las diferencias eran significativas, se aplicaron pruebas t pareadas utilizando los valores de RMSE de los diez pliegues. Se encontraron diferencias significativas entre el modelo base y el enriquecido ($p = 5.49\times10^{-6}$), así como entre el modelo base y el optimizado ($p = 0.0112$). El modelo optimizado también mostró diferencias significativas frente al enriquecido completo ($p = 0.0037$), indicando que reducir variables mejora su estabilidad sin recuperar totalmente la precisión del modelo base.

La Tabla~\ref{tab:comparacion_modelos_final} resume el desempeño de los tres modelos evaluados. Aunque el modelo base conserva la mayor precisión, el modelo optimizado ofrece un balance adecuado entre simplicidad, interpretabilidad y error, y constituye una alternativa eficiente dentro del conjunto enriquecido.

\begin{table}[ht]
\centering
\small
\caption{Comparación de desempeño entre los modelos evaluados (valores en millones de pesos).}
\label{tab:comparacion_modelos_final}
\begin{tabular}{lcccc}
\hline
\textbf{Modelo} & \textbf{RMSE (M\$)} & \textbf{MAE (M\$)} & \textbf{$R^2$} & \textbf{p-valor vs. Base} \\
\hline
Base                         & 0.151 & 0.107 & 0.965 & --- \\
Enriquecido                  & 0.170 & 0.126 & 0.956 & $5.49\times10^{-6}$ \\
Enriquecido optimizado       & 0.158 & 0.114 & 0.960 & 0.0112 \\
\hline
\end{tabular}
\end{table}
