...
\begin{figure}[h]
    ...
    \caption{Etapas generales de la metodología desarrollada}
...
\begin{itemize}
    \item \textbf{Datos de inmuebles (base):} Se utilizaron registros provenientes del repositorio público \texttt{builker-col/bogota-apartments}, con datos correspondientes a agosto de 2024. Los archivos originales en formato JSON fueron transformados a CSV para su procesamiento.
...
La Figura~\ref{fig:log_area} y la Figura~\ref{fig:log_precio} muestran la diferencia en la distribución de ambas variables antes y después de la transformación.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/log_area.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/log_precio.png}
    \caption{Distribución del precio de venta antes y después de la transformación logarítmica}
    \label{fig:log_precio}
\end{figure}
...
\subsection*{Exposición de resultados y reproducibilidad}

El modelo final se expuso mediante un servicio \texttt{FastAPI} con el endpoint \texttt{/api/estimate}, que recibe las características de un inmueble y devuelve su estimación junto con indicadores contextuales.  
Todo el proceso fue documentado en notebooks reproducibles y scripts modulares (Python / PostGIS).  
Las semillas aleatorias y la configuración de validación fueron fijadas para garantizar replicabilidad.  
Los modelos entrenados y datasets intermedios fueron versionados en el directorio \texttt{analisis/data/}.
